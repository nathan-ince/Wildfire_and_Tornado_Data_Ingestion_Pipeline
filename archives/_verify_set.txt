...
def verify_set(
  ...
  if not isinstance(entity, set):
    ...
    if track_entity_target_type: kwargs["entity_target_type"] = set.__name__
    if extra_keyword_arguments: kwargs.update(extra_keyword_arguments)
    error = AbstractError("failed to verify set", "entity type is invalid", **kwargs)
  ...
  if verify_elements:
    if elements_target_type is None:
      ...
      if extra_keyword_arguments: kwargs.update(extra_keyword_arguments)
      error = AbstractError("failed to verify set", "verify_elements is True but elements_target_type is None", **kwargs)
      if log_error: logger.error(error.message, extra=error.kwargs)
      raise error
    for element in entity:
      if not isinstance(element, elements_target_type):
        ...
        if extra_keyword_arguments: kwargs.update(extra_keyword_arguments)
        error = AbstractError("failed to verify set", "element type is invalid", **kwargs)
        if log_error: logger.error(error.message, extra=error.kwargs)
        raise error
  return True
